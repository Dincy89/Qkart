    ...
    for(let i=0;i<cartData.length;i++){
      for(let j=0;j<productsData.length;j++){
        if(cartData[i].productId===productsData[j]._id)
          c.push({...productsData[j],...cartData[i]});
      }
    }
  }
  // console.log(c);
...
const Cart = ({
  ...
  return (
    <>
      ...
      { isReadOnly && ( 
           <Box className="cart" style={{padding: "16px 12px"}}>
             <h4 style={{margin: "10px 0"}}>Order Details</h4>
              <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2} style={{marginBottom: "10px"}}>
                 <span>Products</span> 
              ...
              <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2} style={{marginBottom: "10px"}} > 
                ...
                <span>$0</span> </Stack> 
              <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2} style={{margin: "15px 0 10px 0"}} > 
              <span style={{fontWeight: "600", fontSize: "16px"}}>Total</span> 
              <span style={{fontWeight: "600", fontSize: "16px"}}>${getTotalCartValue(items)}</span> 
              </Stack> 
              </Box>
          ) }
    </>
